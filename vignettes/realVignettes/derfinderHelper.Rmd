...
write.bibtex(c(derfinderH = citation('derfinderHelper'), 
...
`derfinderHelper` `r citep(bib[['colladotorres2014derfinderhelper']])` is a small package that was created to speed up the parent package `derfinder`. It contains a single function, `fstats.apply()`, which is used to calculate the F-statistics for a given data matrix, null and an alternative models.

The data is generally arranged in an matrix where the rows ($n$) are the genomic features of interest (gene level summaries, exon level summaries, or base-pair data) and the columns ($m$) represent the samples. The other two main arguments for `fstats.apply()` are the null and alternative model matrices which are $m x p_0$ and $m x p$  where $p_0$ is the number of covariates in the null model and $p$ is the number of covariates in the alternative model. The models have to be nested and thus by definition $p > p_0$. The end result is a vector of F-statistics with length $n$, which is run length encoded for memory saving purposes.
...
We created `derfinderHelper` for calculating F-statistics using `SnowParam()` from `BiocParallel` `r citep(bib[['morganbiocparallel']])`. Using this form of parallelization requires loading the necessary packages in the child processes. Because `derfinder` takes a long time to load, we shipped off `fstats.apply()` to its own package to improve the speed of the calculations while retaining the memory advantages of `SnowParam()` over `MulticoreParam()`.
